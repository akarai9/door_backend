...
const updateArrData = async (oldVal,newVal,res, MODEL) => {
    try {
        let response = await MODEL.update({'pilRoyaltyObj':{'plfCost':oldVal.plfCost, 'nonPlfRoyalties': oldVal.nonPlfRoyalties , 'workshopDelivery': oldVal.workshopDelivery , '_id': oldVal._id}},
        {$set:{'pilRoyaltyObj':{'plfCost':newVal.plfCost, 'nonPlfRoyalties': newVal.nonPlfRoyalties , 'workshopDelivery': newVal.workshopDelivery , '_id': newVal._id}}})
        return response
    }
    catch (error) {
        res.send(errHandler(error))
