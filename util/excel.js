...
const getPILDataSet = (jsonData, country, month, year) => {
    ...
    // Add Total 
    let excelTotal = {}, excelTotal1 = {}, excelTotal2 = {}, excelTotal3 = {}, excelTotal4 = {},
        excelHeaderValue, excelHeaderValue1, excelHeaderValue2, excelHeaderMOnth, excelHeaderTerritory;
    ...
    excelHeaderValue2 = ['', '', '', '', '', '', '', '', '', '', '', '', '']
    for (let i = 0; i < excelHeader.length; i++) {
        excelTotal1[excelHeader[i]] = excelHeaderValue1[i]
    }
    dataset.push(excelTotal1)

    for (let i = 0; i < excelHeader.length; i++) {
        excelTotal2[excelHeader[i]] = excelHeaderValue2[i]
    }
    dataset.push(excelTotal2)

    excelHeaderMOnth = ['', '', 'Monthly Summary', '', '', '', '', '', 'Instructions', '', '', '', '']

    for (let i = 0; i < excelHeader.length; i++) {
        //debugger;
        excelTotal3[excelHeader[i]] = excelHeaderMOnth[i]
        //debugger;
    }

    dataset.push(excelTotal3)

    excelHeaderTerritory = ['', '', 'Territory Billings to Client', '', '', '', '', '', '1. Fill in client name, invoice#, track facilitated per engagement', '', '', '', '']

    for (let i = 0; i < excelHeader.length; i++) {
        excelTotal4[excelHeader[i]] = excelHeaderTerritory[i]
    }

    // dataset.push(excelTotal4)
...
const getPILHeading = (country, month, year) => {
    ...
    let reportHeading = { value: 'Monthly PIL Turnover Report DOOR', style: CONSTANTS.setExcelStyle('00000000', 14, false, '48689a', 'center') }
    ...
    return [
        ...
        ['Country', country, '', '',
            ...
            ' Workshop Delivery Fee', '', 'Accountability Coaching/Consulting', '', 'Internal Certification Fees collected', ''
        ]
    ]
}
const getPILMerge = () => {
    let columnLength = 13;
    const merges = [
        { start: { row: 2, column: 1 }, end: { row: 3, column: 13 } },
        //{ start: { row: 3, column: 1 }, end: { row: 3, column: 13 } },        
        { start: { row: 4, column: 1 }, end: { row: 4, column: 1 } },
        { start: { row: 4, column: 2 }, end: { row: 4, column: 3 } },
        { start: { row: 4, column: 4 }, end: { row: 4, column: 4 } },
        { start: { row: 4, column: 5 }, end: { row: 4, column: 7 } },
        { start: { row: 4, column: 8 }, end: { row: 4, column: 9 } },
        { start: { row: 4, column: 10 }, end: { row: 4, column: 11 } },
        { start: { row: 4, column: 12 }, end: { row: 4, column: 13 } },
        { start: { row: 27, column: 1 }, end: { row: 27, column: 6 } }
    ]
    return merges
}
